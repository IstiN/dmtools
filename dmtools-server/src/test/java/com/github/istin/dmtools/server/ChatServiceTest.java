...
class ChatServiceTest {
    ...
    void setUp() throws Exception {
        // Configure mock AI to return mock responses (using lenient to avoid unnecessary stubbings warnings)
        lenient().when(mockAI.chat(anyString())).thenReturn("Mock AI response");
        lenient().when(mockAI.chat(anyString(), any(Message[].class))).thenReturn("Mock AI response");
        lenient().when(mockAI.chat(any(Message[].class))).thenReturn("Mock AI response");
    }
    ...
    void testSimpleChatMessageWithoutUserId() throws Exception {
        ...
        assertTrue(response.getError().contains("User authentication is required"));
    }
    ...
    void testSimpleChatMessageWithAutoSelection() throws Exception {
        ...
        try (MockedConstruction<ServerManagedIntegrationsModule> mockedConstruction = 
                mockConstruction(ServerManagedIntegrationsModule.class, 
                    (mock, context) -> when(mock.createAI()).thenReturn(mockAI))) {
    ...
    void testChatWithHistoryWithoutUserId() throws Exception {
        ...
        assertFalse(response.isSuccess());
        assertNotNull(response.getError());
        assertEquals("Failed to process chat request: User authentication is required for AI integration selection", response.getError());
    }
    ...
    void testChatWithEmptyHistory() {
        ...
        assertFalse(response.isSuccess());
        assertNotNull(response.getError());
        assertTrue(response.getError().contains("Failed to process chat request"));
    }
    ...
    void testChatWithInvalidAIIntegration() throws Exception {
        ...
        assertNotNull(response.getError());
        assertTrue(response.getError().contains("Failed to process chat request: Failed to resolve AI integration: Integration not found"));
    ...
    void testChatWithNullAIIntegrationNoUserIntegrations() throws Exception {
        ...
        assertFalse(response.isSuccess());
        assertNotNull(response.getError());
        assertTrue(response.getError().contains("No AI integrations found for user"));
        verify(integrationResolutionHelper).resolveUserFirstAIIntegration(userId);
    }
    ...
    void testChatWithMcpTools_SuccessfulExecution() throws Exception {
        try (MockedConstruction<ServerManagedIntegrationsModule> mockedConstruction = 
            ...
            lenient().when(toolSelectorAgent.run(any()))
    ...
    void testChatWithMcpTools_ToolExecution() throws Exception {
        try (MockedConstruction<ServerManagedIntegrationsModule> mockedIntegrationsModule = 
                mockConstruction(ServerManagedIntegrationsModule.class, 
                    (mock, context) -> when(mock.createAI()).thenReturn(mockAI));
             MockedConstruction<ToolSelectorAgent> mockedToolSelector = 
                mockConstruction(ToolSelectorAgent.class,
                    (mock, context) -> {
